...
- **logger.hpp/cpp** — `ConsoleSink`, `FileSink`, `RingBufferSink`, `Logger`, log macros, fmt-style overloads (`logInfo("cat", "msg={}", x)`), `formatLogMessage` (internal).
...
- **build_info.hpp** — Author hash (`author`), `ObfString`, `ObfPassthrough`, `detail::obf_key` / `obf_xor`.
- **obfuscate.hpp** — `VAPI_OBF` / `VAPI_OBF_LIT` (uses `build_info.hpp`).
